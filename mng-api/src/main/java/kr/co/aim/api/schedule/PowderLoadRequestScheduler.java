...
public class PowderLoadRequestScheduler {
    ...
    @SchedulerLock(name = "powderLoadRequestScheduler",
            lockAtMostFor = "PT2M",     // 작업 최장 소요시간 + 버퍼
            lockAtLeastFor = "PT5S")    // 최소 간격(선택)
    public void powderLoadRequestScheduler() {

        List<Port> portList = portService.findByTransportState(PortTransportState.READY_TO_LOAD.getValue());
        //List<Port> portList = portService.findByTransportStateAndPortRoleType(PortTransportState.READY_TO_LOAD.getValue(),SystemName.WCS.getValue());
