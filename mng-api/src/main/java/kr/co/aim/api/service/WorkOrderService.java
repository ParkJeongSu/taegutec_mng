package kr.co.aim.api.service;

import kr.co.aim.api.dto.WorkOrderCreateRequestDto;
import kr.co.aim.api.dto.WorkOrderResponseDto;
import kr.co.aim.api.dto.WorkOrderSearchConditionDto;
import kr.co.aim.api.dto.WorkOrderUpdateRequestDto;
import kr.co.aim.common.enums.EventName;
import kr.co.aim.common.error.EntityNotFoundException;
import kr.co.aim.common.record.TransactionInfo;
import kr.co.aim.domain.command.WorkOrderCreateCommand;
import kr.co.aim.domain.command.WorkOrderUpdateCommand;
import kr.co.aim.domain.model.WorkOrder;
import kr.co.aim.domain.repository.WorkOrderRepository;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import java.util.List;
import java.util.Optional;

@Service
@RequiredArgsConstructor // final 필드에 대한 생성자를 자동으로 만들어줍니다. (DI)
@Slf4j
public class WorkOrderService {
    private final WorkOrderRepository workOrderRepository;

    /**
     * 사용자의 데이터를 생성합니다.
     * @param requestDto 사용자의 생성 데이터
     * @return 생성된 사용자 도메인 객체
     */
    @Transactional // 이 메소드가 하나의 트랜잭션으로 동작하도록 보장합니다.
    public WorkOrder createWorkOrder(WorkOrderCreateRequestDto requestDto) {
        // 1. Repository를 통해 Domain 객체를 가져온다.
//        Optional<WorkOrder> optionalWorkOrder = workOrderRepository.findByWorkOrderName(requestDto.getWorkOrderName());
//        if(optionalWorkOrder.isPresent()){
//            throw new EntityExistException("이미 생성된 Lot 이름입니다. ID: " + requestDto.getWorkOrderName());
//        }

        String eventName = EventName.CREATED.getValue();

        TransactionInfo tx = TransactionInfo.now(eventName,requestDto.getEventUser(),requestDto.getEventComment());
        WorkOrderCreateCommand command =
                WorkOrderCreateCommand.builder()
                        .workOrderName(requestDto.getWorkOrderName())
                        .lotName(requestDto.getLotName())
                        .description(requestDto.getDescription())
                        .vendorName(requestDto.getVendorName())
                        .productDefName(requestDto.getProductDefName())
                        .processFlowName(requestDto.getProcessFlowName())
                        .processOperationName(requestDto.getProcessOperationName())
                        .recipeName(requestDto.getRecipeName())
                        .workOrderState(requestDto.getWorkOrderState())
                        .holdState(requestDto.getHoldState())
                        .reasonCode(requestDto.getReasonCode())
                        .equipmentName(requestDto.getEquipmentName())
                        .planQuantity(requestDto.getPlanQuantity())
                        .createdQuantity(requestDto.getCreatedQuantity())
                        .releasedQuantity(requestDto.getReleasedQuantity())
                        .finishedQuantity(requestDto.getFinishedQuantity())
                        .scrappedQuantity(requestDto.getScrappedQuantity())
                        .workOrderCount(requestDto.getWorkOrderCount())
                        .createTime(requestDto.getCreateTime())
                        .releaseTime(requestDto.getReleaseTime())
                        .completeTime(requestDto.getCompleteTime())
                        .createUser(requestDto.getCreateUser())
                        .releaseUser(requestDto.getReleaseUser())
                        .completeUser(requestDto.getCompleteUser())
                        .dueDate(requestDto.getDueDate())
                        .transactionInfo(tx)
                        .build();

        WorkOrder workOrder = WorkOrder.create(command);

        return workOrderRepository.save(workOrder);
    }

    @Transactional(readOnly = true)
    public Page<WorkOrderResponseDto> findWorkOrder(WorkOrderSearchConditionDto condition, Pageable pageable) {
        //1. Repository에서 Page<Entity>를 조회합니다.

        Page<WorkOrderResponseDto> page = null;//workOrderRepository.findWorkOrderWithConditions(condition,pageable);

        return page;
    }

    /**
     * 사용자의 데이터를 변경합니다.
     * @param requestDto 사용자의 변경 데이터
     * @return 변경된 사용자 도메인 객체
     */
    @Transactional // 이 메소드가 하나의 트랜잭션으로 동작하도록 보장합니다.
    public WorkOrder changeWorkOrder(Long id, WorkOrderUpdateRequestDto requestDto) {
        // 1. Repository를 통해 Domain 객체를 가져온다.
        WorkOrder workOrder;
        Optional<WorkOrder> optionalWorkOrder = workOrderRepository.findById(id);
        if(optionalWorkOrder.isPresent()){
            workOrder = optionalWorkOrder.get();
        }
        else {
            throw new EntityNotFoundException("존재하지 않는 설정입니다. ID: " + requestDto.getId());
        }
        String eventName = EventName.UPDATED.getValue();

        TransactionInfo tx = TransactionInfo.now(eventName,requestDto.getEventUser(),requestDto.getEventComment());
        WorkOrderUpdateCommand command =
                WorkOrderUpdateCommand.builder()
                        .transactionInfo(tx)
                        .build();

        workOrder.changeWorkOrder(command);

        return workOrderRepository.save(workOrder);
    }


    @Transactional
    public void deleteAllByIdInBatch(List<Long> ids) {
        if (ids == null || ids.isEmpty()) {
            return; // 삭제할 ID가 없으면 아무 작업도 하지 않음
        }
        // 여러 건을 삭제할 때는 이 메서드가 성능상 가장 효율적입니다.
        // DELETE ... WHERE id IN (...) 쿼리를 한 번에 실행합니다.
        workOrderRepository.deleteAllByIdInBatch(ids);
    }
}
