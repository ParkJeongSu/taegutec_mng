...
@Profile({"pex","tex"})
public class CarrierScannedHandler implements MessageHandler<String> {
    ...
    public Object handle(String message) {
        ...
        // 1. 자신에게 맞는 DTO로 역직렬화
        TypeReference<BaseMessage<CarrierScannedBody>> typeRef = new TypeReference<>() {};
        BaseMessage<CarrierScannedBody> requestMessage = objectMapper.readValue(message, typeRef);

        // 2. 해당 비즈니스 로직 호출
        // 서비스 호출
        messageExecuteService.carrierScanned(requestMessage);
