...
import kr.co.aim.common.format.ActiveTransportJobReportBody;
...
public class ActiveTransportJobReportHandler implements MessageHandler<String> {
    ...
    public String getSupportedMessageName() {
        return MessageList.ACTIVE_TRANSPORT_JOB_REPORT.getMessageName();
    }
    ...
    public Object handle(String message) {
        ...
        // 1. 자신에게 맞는 DTO로 역직렬화
        TypeReference<BaseMessage<ActiveTransportJobReportBody>> typeRef = new TypeReference<>() {};
        BaseMessage<ActiveTransportJobReportBody> requestMessage = objectMapper.readValue(message, typeRef);

        // 2. 해당 비즈니스 로직 호출
        // 서비스 호출
        messageExecuteService.activeTransportJobReport(requestMessage);
