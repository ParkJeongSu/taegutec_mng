...
import kr.co.aim.common.format.CarrierCleanJobCanceledBody;
...
public class CarrierCleanJobCanceledHandler implements MessageHandler<String> {
    ...
    public String getSupportedMessageName() {
        return MessageList.CARRIER_CLEAN_JOB_CANCELED.getMessageName();
    }
    ...
    public Object handle(String message) {
        ...
        // 1. 자신에게 맞는 DTO로 역직렬화
        TypeReference<BaseMessage<CarrierCleanJobCanceledBody>> typeRef = new TypeReference<>() {};
        BaseMessage<CarrierCleanJobCanceledBody> requestMessage = objectMapper.readValue(message, typeRef);

        // 2. 해당 비즈니스 로직 호출
        // 서비스 호출
        messageExecuteService.carrierCleanJobCanceled(requestMessage);
