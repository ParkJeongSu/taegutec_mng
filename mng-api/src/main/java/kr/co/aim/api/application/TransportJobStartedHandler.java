...
import kr.co.aim.common.format.TransportJobStartedBody;
...
public class TransportJobStartedHandler implements MessageHandler<String> {
    ...
    public String getSupportedMessageName() {
        return MessageList.TRANSPORT_JOB_STARTED.getMessageName();
    }
    ...
    public Object handle(String message) {
        ...
        // 1. 자신에게 맞는 DTO로 역직렬화
        TypeReference<BaseMessage<TransportJobStartedBody>> typeRef = new TypeReference<>() {};
        BaseMessage<TransportJobStartedBody> requestMessage = objectMapper.readValue(message, typeRef);

        // 2. 해당 비즈니스 로직 호출
        // 서비스 호출
        messageExecuteService.transportJobStarted(requestMessage);
