...
public class UnloadCompletedHandler implements MessageHandler<String> {
    ...
    public String getSupportedMessageName() {
        return MessageList.UNLOAD_COMPLETE.getMessageName();
    }
    ...
    public Object handle(String message) {
        ...
        // 1. 자신에게 맞는 DTO로 역직렬화
        TypeReference<BaseMessage<UnLoadCompletedBody>> typeRef = new TypeReference<>() {};
        BaseMessage<UnLoadCompletedBody> request = objectMapper.readValue(message, typeRef);

        // 2. 해당 비즈니스 로직 호출
        // 서비스 호출
        messageExecuteService.unLoadCompleted(request);
