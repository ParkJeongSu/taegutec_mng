...
import kr.co.aim.common.format.TransportJobValidationRequestBody;
...
@Profile({"tex"})
public class TransportOrderValidationRequestHandler implements MessageHandler<String> {
    ...
    public String getSupportedMessageName() {
        return MessageList.TRANSPORT_ORDER_VALIDATION_REQUEST.getMessageName();
    }
    ...
    public Object handle(String message) {
        ...
        // 서비스 호출
        BaseMessage<TransportJobValidationRequestBody> transportJobValidationRequestBodyBaseMessage = messageExecuteService.transportOrderValidationRequest(requestMessage);

        jsonUtils.writePrettyJson(transportJobValidationRequestBodyBaseMessage);

        if(ObjectUtils.isEmpty(transportJobValidationRequestBodyBaseMessage)){
            log.info("transportJobRequestBodyBaseMessage is null");
        }
        else if(ObjectUtils.isNotEmpty(transportJobValidationRequestBodyBaseMessage)){
            rabbitTemplate.convertAndSend(
                    RabbitConfig.EXCHANGE_WCS,
                    RabbitConfig.ROUTING_WCS,
                    transportJobValidationRequestBodyBaseMessage );
        }
