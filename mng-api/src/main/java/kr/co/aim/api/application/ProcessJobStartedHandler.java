...
public class ProcessJobStartedHandler implements MessageHandler<String> {
    ...
    public String getSupportedMessageName() {
        return MessageList.PROCESS_JOB_STARTED.getMessageName();
    }
    ...
    public Object handle(String message) {
        ...
        // 1. 자신에게 맞는 DTO로 역직렬화
        TypeReference<BaseMessage<ProcessJobStartedBody>> typeRef = new TypeReference<>() {};
        BaseMessage<ProcessJobStartedBody> requestMessage = objectMapper.readValue(message, typeRef);

        messageExecuteService.processJobStarted(requestMessage);
        return null;
    }
}