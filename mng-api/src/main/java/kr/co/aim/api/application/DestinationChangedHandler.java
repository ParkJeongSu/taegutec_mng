...
public class DestinationChangedHandler implements MessageHandler<String> {
    ...
    public String getSupportedMessageName() {
        return MessageList.DESTINATION_CHANGED.getMessageName();
    }
    ...
    public Object handle(String message) {
        ...
        // 1. 자신에게 맞는 DTO로 역직렬화
        TypeReference<BaseMessage<DestinationChangedBody>> typeRef = new TypeReference<>() {};
        BaseMessage<DestinationChangedBody> requestMessage = objectMapper.readValue(message, typeRef);

        // 2. 해당 비즈니스 로직 호출
        // 서비스 호출
        messageExecuteService.destinationChanged(requestMessage);
