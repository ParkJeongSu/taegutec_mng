...
public interface TransportOrderMapper {
    ...
    TransportOrder toDomain(TransportOrderEntity entity);

    TransportOrderEntity toEntity(TransportOrder domain);

    @Mapping(target = "id", expression = "java(TsidUtils.nextId())") // [3] 자바 코드 호출!
    TransportOrderHistoryEntity toHistoryEntity(TransportOrder domain);
